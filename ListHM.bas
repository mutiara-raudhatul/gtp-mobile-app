...
Sub Globals
	...
	Private idHM As Label
	Private nameHM As Label
	Private addressHM As Label
	Private ivHM As B4XView
...
Private Sub CreateItem (id As String, name As String, address As String,  img As String) As B4XView
	...
	idHM.Text = id
	nameHM.Text = name
	addressHM.Text = address
	
	Dim imgPath As String = Main.server & "media/photos/homestay" &img
	imgMap.Put(ivHM, imgPath)
	Return p
End Sub
				...
				Try
					...
					If data.Size-1 > 0 Then
						...
						For i = 0 To data.Size-1
							Dim rg As Map
							rg = data.Get(i)
							CLV.Add(CreateItem(rg.Get("id"), rg.Get("name"), rg.Get("address"), rg.Get("gallery")), "")
...
Private Sub PanelListItem_Click
	...
	Log("Redirect to DetailHM Activity")
	StartActivity(DetailHM)
End Sub