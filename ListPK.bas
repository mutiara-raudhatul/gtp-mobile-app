...
Sub Globals
	...
	Private idPK As Label
	Private namePK As Label
	Private nametypePK As Label
	Private pricePK As Label
...
Sub Activity_Create(FirstTime As Boolean)
	...
	CLV.Clear
	imgMap.Clear
	Core.ExecuteUrlGet(Main.API&"package", "LoadPK", Me)
End Sub
...
Private Sub PanelListItem_Click
	...
	Log("Redirect to DetailPK Activity")
	StartActivity(DetailPK)
End Sub

Private Sub CreateItem (id As String, name As String, price As String, type_name As String, img As String) As B4XView
	...
	idPK.Text = id
	namePK.Text = name
	nametypePK.Text = type_name
	pricePK.Text = price
	
	Dim imgPath As String = Main.server & "media/photos/package/" &img
				...
				Try
					...
					If data.Size-1 > 0 Then
						...
						For i = 0 To data.Size-1
							Dim ev As Map
							ev = data.Get(i)
							CLV.Add(CreateItem(ev.Get("id"), ev.Get("name"),  ev.Get("price"),  ev.Get("type_name"), ev.Get("gallery")), "")
